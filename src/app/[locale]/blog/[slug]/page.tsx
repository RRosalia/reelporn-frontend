...
function BlogDetailPage() {
    ...
    const loadPost = async () => {
        try {
            ...
            if (postData.category) {
                const related = await BlogService.getAllPosts(1, 3, postData.category.slug);
    ...
    if (error || !post) {
        return (
            <div style={{ background: 'linear-gradient(135deg, #1a1626 0%, #2b2838 100%)', minHeight: '100vh', paddingTop: '80px' }}>
                <div className="container mx-auto px-4 py-12 text-center">
                    <h1 className="text-white text-4xl font-bold mb-4">{t('blog.notFound')}</h1>
                    <p className="text-gray-400 mb-8">{t('blog.notFoundDescription')}</p>
                    <Link href={"/blog" as any} className="inline-block px-6 py-3 bg-pink-500 text-white rounded hover:bg-pink-600 transition-colors">
                        {t('blog.backToBlog')}
                    </Link>
                </div>
            </div>
            ...
            <div className="container mx-auto px-4 py-8">
                ...
                {relatedPosts.length > 0 && (
                    <div className="max-w-6xl mx-auto mt-16">
                        <h2 className="text-white text-3xl font-bold mb-8">{t('blog.relatedPosts')}</h2>
